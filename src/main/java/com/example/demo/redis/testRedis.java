...
public class testRedis {
    ...
    public static void textList() {
        ...
        jedis.lpush("lst", "List01", "List02","List03");
        System.out.println("List中第一个数据为：" + jedis.lindex("lst", 0));
        System.out.println("List中第二个数据为：" + jedis.lindex("lst", 1));
        System.out.println("List中第二个数据为：" + jedis.lindex("lst", 2));
        System.out.println("List中第二个数据为：" + jedis.lindex("lst", 8));
        jedis.close();
    }
...
//		textSortSet();
		 //连接本地的 Redis 服务
        Jedis jedis = new Jedis("localhost");
        System.out.println(jedis.get("a"));//执行结果：xinxin  
        ...
        System.out.println(jedis.srandmember("user"));  // 命令用于返回集合中的一个随机元素。 
